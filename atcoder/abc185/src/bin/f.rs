...
impl SegmentTree {
    ...
    pub fn update(&mut self, x: usize, val: usize) {
        ...
        while (ind > 0) {
            ind = (ind - 1) / 2;
            self.node[ind] = self.node[2 * ind + 1] ^ self.node[2 * ind + 2];
        }
    }
...
fn main() {
    ...
    let mut segtree = SegmentTree::new(A.clone(), 0);

    for (t, x, y) in TXY.into_iter() {
        if t == 1 {
            segtree.update(x, y ^ A[x]);
            A[x] = y ^ A[x];
        } else {
            println!("{}", segtree.query(x, y))
        }
    }
}
